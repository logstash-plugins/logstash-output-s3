...
NOTE: If you're using multiple S3 outputs than make sure to always set
<<plugins-{type}s-{plugin}-temporary_directory,`temporary_directory`>> to a
...
per output as all of the directory's contents gets processed and deleted upon recovery, and may cause data loss
